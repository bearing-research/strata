...
# ---------------------------------------------------------------------------
# Attached-backend 409 enforcement (env Phase 2)
#
# Every mutating env endpoint must refuse with HTTP 409 when the
# notebook is using AttachedBackend (read-only). The env-status payload
# must also expose ``backend`` + ``supports_mutations`` so the frontend
# can disable the corresponding buttons up front.


def _open_attached_notebook(client: TestClient, tmpdir: Path) -> str:
    """Create a notebook, set ``[strata] backend = "attached"``, open it.

    Returns the session_id. Used by the 409 tests below to share setup.
    """
    notebook_dir = create_notebook(tmpdir, "Attached Backend Test")
...
def test_environment_status_exposes_backend_and_capability():
    """Open payload's ``environment`` block should carry the new backend
    fields so the frontend can render the badge + button state without
    a second round-trip."""
    client = TestClient(create_test_app())
...
def test_environment_sync_rejected_on_attached_backend():
    client = TestClient(create_test_app())
    with tempfile.TemporaryDirectory() as tmpdir:
        session_id = _open_attached_notebook(client, Path(tmpdir))
        response = client.post(f"/v1/notebooks/{session_id}/environment/sync")
...
def test_environment_job_rejected_on_attached_backend():
    client = TestClient(create_test_app())
    with tempfile.TemporaryDirectory() as tmpdir:
        session_id = _open_attached_notebook(client, Path(tmpdir))
        response = client.post(
            f"/v1/notebooks/{session_id}/environment/jobs",
            json={"action": "add", "package": "requests"},
        )
        assert response.status_code == 409


def test_add_dependency_rejected_on_attached_backend():
    client = TestClient(create_test_app())
    with tempfile.TemporaryDirectory() as tmpdir:
        session_id = _open_attached_notebook(client, Path(tmpdir))
        response = client.post(
            f"/v1/notebooks/{session_id}/dependencies",
            json={"package": "requests"},
        )
        assert response.status_code == 409


def test_remove_dependency_rejected_on_attached_backend():
    client = TestClient(create_test_app())
    with tempfile.TemporaryDirectory() as tmpdir:
        session_id = _open_attached_notebook(client, Path(tmpdir))
        response = client.delete(f"/v1/notebooks/{session_id}/dependencies/requests")
        assert response.status_code == 409


def test_requirements_import_rejected_on_attached_backend():
    client = TestClient(create_test_app())
    with tempfile.TemporaryDirectory() as tmpdir:
        session_id = _open_attached_notebook(client, Path(tmpdir))
        response = client.post(
            f"/v1/notebooks/{session_id}/environment/requirements.txt",
            json={"requirements": "pandas\n"},
        )
        assert response.status_code == 409


def test_environment_yaml_import_rejected_on_attached_backend():
    client = TestClient(create_test_app())
    with tempfile.TemporaryDirectory() as tmpdir:
        session_id = _open_attached_notebook(client, Path(tmpdir))
        response = client.post(
            f"/v1/notebooks/{session_id}/environment/environment.yaml",
            json={"environment_yaml": "name: x\ndependencies: [pandas]\n"},
        )
        assert response.status_code == 409
