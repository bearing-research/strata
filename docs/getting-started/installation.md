...
| `uv run strata agent <notebook-dir>` | One-command on-ramp for driving a notebook with a coding agent: create-or-open the notebook, start (or reuse) a server with the MCP endpoint enabled, open a session, write the `.mcp.json` + `CLAUDE.md` the agent needs, and attach the TUI. Then run `claude` in the notebook directory to drive it live. Needs the `[mcp]` and `[tui]` extras. See [Driving a notebook with a coding agent](../notebook/agent.md). |
