...
| --- | --- | --- | --- | --- |
| 1 | GC deletes a notebook's current value during a rebuild | `tests/test_artifact_store.py::TestGcSparesCurrentValues` | GC also protects the latest ready/superseded version ✓ | **Fixed** (ready only until 2 lands) |
| 2 | Promoting one notebook's output strands another's | `test_artifact_counterexamples.py::test_cross_id_dedup` | `get_latest_version` accepts `superseded` ✓ | Audit the other callers first |
